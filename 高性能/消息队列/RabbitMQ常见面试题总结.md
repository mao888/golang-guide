...
## [#](#说说生产者-producer-和消费者-consumer) **说说生产者 Producer 和消费者 Consumer?**
...
## [#](#说说-broker-服务节点、queue-队列、exchange-交换器) 说说 Broker 服务节点、Queue 队列、Exchange 交换器？
...
## [#](#什么是延迟队列-rabbitmq-怎么实现延迟队列) 什么是延迟队列？RabbitMQ 怎么实现延迟队列？
...
## [#](#rabbitmq-有哪些工作模式) RabbitMQ 有哪些工作模式？
...
## [#](#rabbitmq-消息怎么传输) RabbitMQ 消息怎么传输？
...
## [#](#如何保证-rabbitmq-消息的顺序性) 如何保证 RabbitMQ 消息的顺序性？

- 拆分多个 queue(消息队列)，每个 queue(消息队列) 一个 consumer(消费者)，就是多一些 queue (消息队列)而已，确实是麻烦点；
- 或者就一个 queue (消息队列)但是对应一个 consumer(消费者)，然后这个 consumer(消费者)内部用内存队列做排队，然后分发给底层不同的 worker 来处理。

## [#](#如何保证-rabbitmq-高可用的) 如何保证 RabbitMQ 高可用的？
...
## [#](#如何解决消息队列的延时以及过期失效问题) 如何解决消息队列的延时以及过期失效问题？
