...
![](https://cdn.nlark.com/yuque/0/2024/webp/22219483/1722529207049-2182fadd-f26f-4de7-aee6-8a7321018c7a.webp#averageHue=%23f1f1f1&clientId=u59f54e0e-23a7-4&from=paste&id=u007073c1&originHeight=160&originWidth=1080&originalType=url&ratio=1&rotation=0&showTitle=false&status=done&style=none&taskId=u802fc3f1-ecd4-401c-b4c5-2b7e1a26efa&title=)
### Redis 是怎么实现惰性删除的
...
## Redis内存淘汰策略
内存淘汰策略是解决内存过大的问题，当 Redis 的运行内存超过最大运行内存时，就会触发内存淘汰策略，Redis 4.0 之后共实现了 8 种内存淘汰策略，这 8 种的策略进行分类，如下：
![](https://cdn.nlark.com/yuque/0/2024/webp/22219483/1722529260142-71e13e48-6a98-47c9-a040-5381a98726da.webp#averageHue=%23f6f6f6&clientId=u59f54e0e-23a7-4&from=paste&id=u95682a21&originHeight=342&originWidth=1080&originalType=url&ratio=1&rotation=0&showTitle=false&status=done&style=none&taskId=u8c38331d-177b-49b1-b969-1169002a3bd&title=)
...
![](https://cdn.nlark.com/yuque/0/2024/webp/22219483/1722529686100-9bd0bbf3-33ab-4876-ab28-e434ecb70dc5.webp#averageHue=%23a49e95&clientId=u59f54e0e-23a7-4&from=paste&id=ub8fd8d94&originHeight=222&originWidth=1080&originalType=url&ratio=1&rotation=0&showTitle=false&status=done&style=none&taskId=u2c3ea4de-93a3-47d3-b453-eaa6596f820&title=)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160428805-1abf4c01-4f82-4216-89b2-278ff06ca689.png#averageHue=%23d1f2f2&clientId=u9081c6a5-99f0-4&from=paste&id=uf57700b0&originHeight=581&originWidth=1310&originalType=url&ratio=1&rotation=0&showTitle=false&size=248824&status=done&style=none&taskId=u5b84a205-3318-4ea7-8571-0a97c7e97e0&title=)
**优点：**
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160428767-06cbc712-fcf7-4f43-8b15-0a5254e7555b.png#averageHue=%23f9faf7&clientId=u9081c6a5-99f0-4&from=paste&id=uedac16a4&originHeight=530&originWidth=1339&originalType=url&ratio=1&rotation=0&showTitle=false&size=252976&status=done&style=none&taskId=u946529c3-31b7-40a3-8d2c-ce5beb16401&title=)
**优点：**
...
1. **RDB 快照**：主节点生成 RDB 文件（内存快照）并发送给从节点，用于初次同步。
2. **复制流**：主节点将增量写操作日志发送给从节点，保持数据实时同步。
3. **偏移量和复制ID**：主从节点通过复制偏移量和复制ID（replication ID）来跟踪同步状态，确保从节点可以从正确的位置继续复制。
#### 故障切换
...
5. **从节点故障**：
   ...
   - 故障恢复后，从节点会自动重新同步数据。
### 总结
Redis 主从复制通过初次同步和增量同步机制确保从节点与主节点的数据一致。通过主从复制，Redis 实现了读写分离和数据冗余，增强了系统的高可用性和扩展性。哨兵机制进一步保证了主节点故障时的自动故障切换和恢复。
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160428866-a1fe80bd-214b-4d45-a49c-2d48bb7c06dd.png#averageHue=%23e3e7de&clientId=u9081c6a5-99f0-4&from=paste&id=u5302e453&originHeight=1140&originWidth=817&originalType=url&ratio=1&rotation=0&showTitle=false&size=326165&status=done&style=none&taskId=u7a3005bc-5506-42d5-9bd6-cd9be19f400&title=)
**优点：**
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160429207-9690daa9-7760-4aeb-9789-0e64a37eda1c.png#averageHue=%23d0ddca&clientId=u9081c6a5-99f0-4&from=paste&id=ue5574b74&originHeight=668&originWidth=1344&originalType=url&ratio=1&rotation=0&showTitle=false&size=649766&status=done&style=none&taskId=u69b4a480-18b7-4930-ad71-1b3660a1aab&title=)
**优点：**
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160429050-071f8e60-364a-4ace-8686-e42afe970e14.png#averageHue=%23deeadd&clientId=u9081c6a5-99f0-4&from=paste&id=u9baac34b&originHeight=1152&originWidth=854&originalType=url&ratio=1&rotation=0&showTitle=false&size=488913&status=done&style=none&taskId=u511eea42-9c71-41c3-a309-e30b35c26c0&title=)
**优点：**
...
算法示意图如下：
![image.png](https://cdn.nlark.com/yuque/0/2022/png/22219483/1647160429840-071ed33a-f699-433a-afa0-1783c7554e57.png#averageHue=%23fefefe&clientId=u9081c6a5-99f0-4&from=paste&id=u6e05bbf4&originHeight=698&originWidth=648&originalType=url&ratio=1&rotation=0&showTitle=false&size=48952&status=done&style=none&taskId=ue33376a0-d069-418a-b9e7-921fa4e76ab&title=)
# 其他
## 59. Redis如何做内存优化？
...
1. **减少数据库负载**：
   - 将频繁访问的数据存储在缓存中，减少数据库查询次数，降低数据库负载。
   - 例如，将用户会话数据、配置数据和热门商品信息存储在缓存中。
2. **加快数据访问速度**：
   - 缓存通常位于内存中，访问速度比磁盘存储快得多，从而加快数据读取速度。
   - 例如，使用 Redis 或 Memcached 作为缓存层，存储需要快速访问的数据。
3. **提高系统的可扩展性**：
   - 分布式缓存能够水平扩展，通过增加缓存节点来处理更多的请求和存储更多的数据。
   - 例如，使用 Redis 集群或 Memcached 集群来分布式存储和处理数据。
...
### 应对热点问题
热点问题是指某个键或一小部分键被频繁访问，导致缓存节点的负载过高，影响整体性能。
**解决方法**：
...
1. **分批删除**：
```go
func deleteLargeKey(redisClient *redis.Client, key string, batchSize int64) {
    cursor := uint64(0)
    for {
        var keys []string
        var err error
        cursor, keys, err = redisClient.Scan(cursor, key+"*", batchSize).Result()
        if err != nil {
...
2. **异步删除**：
```go
func asyncDeleteLargeKey(redisClient *redis.Client, key string) {
    redisClient.Unlink(key)
}
```
### 注意事项

1. **监控 Redis 负载**：在执行大key删除操作时，监控 Redis 的 CPU 和内存使用情况，确保不会影响系统稳定性。
2. **测试删除脚本**：在生产环境执行前，在测试环境中验证删除脚本的效果和性能，确保安全。
3. **备份数据**：在删除关键数据前，确保已进行数据备份，以防误删造成数据丢失。

---

这样可以让面试官对你解决大key删除问题的思路和具体实现有清晰的了解。
