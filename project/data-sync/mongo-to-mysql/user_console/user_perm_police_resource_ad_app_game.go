...
func RunUserPermPoliceResourceAdAppAndGame() {
	...
	for _, s := range arkIDAndAppIdAndGameIdStrArr {
		...
		if len(userPerm) != 0 {
			...
			err := db2.MySQLClientUser.Table("user_perm").Where("user_id = ?", arkID).Delete(&bean.UserPerm{})
			if err != nil {
				fmt.Println("user_perm 删除失败，user_id：", arkID)
				return
			}
			// 根据 scope_id 删除 policy_resource表
			err = db2.MySQLClientUser.Table("policy_resource").Where("policy_id = ?", userPerm[0].ScopeID).Delete(&bean.PolicyResource{})
			if err != nil {
				fmt.Println("policy_resource 删除失败，policy_id：", userPerm[0].ScopeID)
				return
			}
			// 根据 scope_id 删除 policy表
			err = db2.MySQLClientUser.Table("policy").Where("id = ?", userPerm[0].ScopeID).Delete(&bean.Policy{})
			if err != nil {
		...
		err = db2.MySQLClient.Table("app").Where("game_id in ?", gameIDStrArr).Distinct("app_id").Find(&game).Error
		if err != nil {
			fmt.Println("根据app_id查询game 错误：", err)
		}
		// 将 根据 game_id 查询 app_id 与原 app_id 数组合并
		for _, app := range game {
			appIDStrArr = append(appIDStrArr, string(app.ID))
		}
		// 将合并后的app_id数组去重
