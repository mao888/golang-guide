...
class AssetCenterModel extends BaseModel {
    ...
    getSchema() {
        return {
            ...
            artneed_id: String,
            asset_long_name: String,//文件名称
            asset_name: String,//素材内容标题
    ...
    async save(data) {
        data._id = await require("../CruiserIdGenModel").genId('cruiser_assets', 1000000);
        return super.save(data);
    }

    async insertMany(arr) {
        for (let item of arr) {
            item._id = await require("../CruiserIdGenModel").genId('cruiser_assets', 1000000);
        }
