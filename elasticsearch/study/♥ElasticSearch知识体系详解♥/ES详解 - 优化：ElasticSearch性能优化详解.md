...
- ES详解 - 优化：ElasticSearch性能优化详解
  ...
  - 索引优化设置
    - [批量提交](#批量提交)
    - [增加 Refresh 时间间隔](#增加-refresh-时间间隔)
    - [修改 index_buffer_size 的设置](#修改-index_buffer_size-的设置)
    - [修改 translog 相关的设置](#修改-translog-相关的设置)
    - [注意 _id 字段的使用](#注意-_id-字段的使用)
    - [注意 _all 字段及 _source 字段的使用](#注意-_all-字段及-_source-字段的使用)
    - [合理的配置使用 index 属性](#合理的配置使用-index-属性)
    - [减少副本数量](#减少副本数量)
  - 查询方面优化
    - 路由优化
      - [不带 routing 查询](#不带-routing-查询)
      - [带 routing 查询](#带-routing-查询)
    - [Filter VS Query](#filter-vs-query)
    - [深度翻页](#深度翻页)
    - [脚本（script）合理使用](#脚本script合理使用)
    - [Cache的设置及使用](#cache的设置及使用)
    - [更多查询优化经验](#更多查询优化经验)
    - [通过开启慢查询配置定位慢查询](#通过开启慢查询配置定位慢查询)
  - 数据结构优化
    - [尽量减少不需要的字段](#尽量减少不需要的字段)
    - [Nested Object vs Parent/Child](#nested-object-vs-parentchild)
    - [选择静态映射，非必需时，禁止动态映射](#选择静态映射非必需时禁止动态映射)
    - [document 模型设计](#document-模型设计)
  - 集群架构设计
    - 主节点、数据节点和协调节点分离
      - [主（master）节点](#主master节点)
      - [数据（data）节点](#数据data节点)
    - [关闭 data 节点服务器中的 http 功能](#关闭-data-节点服务器中的-http-功能)
    - [一台服务器上最好只部署一个 node](#一台服务器上最好只部署一个-node)
    - [集群分片设置](#集群分片设置)
...
相关原理，请参考[ES详解 - 原理：ES原理之索引文档流程详解]()

### [#](#修改-index-buffer-size-的设置) 修改 index_buffer_size 的设置
...
translog我们在[ES详解 - 原理：ES原理之索引文档流程详解]() 也有介绍。

### [#](#注意-id-字段的使用) 注意 _id 字段的使用

_id 字段的使用，应尽可能避免自定义 _id，以避免针对 ID 的版本管理；建议使用 ES 的默认 ID 生成策略或使用数字类型 ID 做为主键。

### [#](#注意-all-字段及-source-字段的使用) 注意 _all 字段及 _source 字段的使用
...
### [#](#nested-object-vs-parent-child) Nested Object vs Parent/Child
...
### [#](#选择静态映射-非必需时-禁止动态映射) 选择静态映射，非必需时，禁止动态映射
...
### [#](#主节点、数据节点和协调节点分离) 主节点、数据节点和协调节点分离
